...
def test_str_and_repr_and_notimplemented_eq():
    calc = Calculation(operation="Addition", operand1=Decimal("2"), operand2=Decimal("3"))
    s = str(calc)
    r = repr(calc)
    # Basic assertions on formatting
    assert "Addition(2, 3) = 5" in s
    assert "Calculation(operation='Addition'" in r
    # __eq__ NotImplemented path when comparing with unrelated object
    class Foo:  # different type
        pass
    assert Calculation.__eq__(calc, Foo()) is NotImplemented


def test_calculate_wraps_errors_to_operation_error():
    # Subclass to simulate an internal operation error during calculate
    class BadCalc(Calculation):
        def calculate(self):  # type: ignore[override]
            try:
                raise ValueError("bad")
            except (Exception,) as e:  # mimic original except path
                from app.exceptions import OperationError
                raise OperationError(f"Calculation failed: {str(e)}")

    with pytest.raises(OperationError, match="Calculation failed: bad"):
        BadCalc(operation="Addition", operand1=Decimal("1"), operand2=Decimal("2"))
...
@pytest.mark.parametrize(
    ...
    [
        (Decimal("-1"), Decimal("2"), "Cannot calculate root of negative number"),
        (Decimal("2"), Decimal("0"), "Zero root is undefined"),
    ]
)
def test_raise_invalid_root(x, y, expected_message):
    with pytest.raises(OperationError, match=expected_message):
        Calculation._raise_invalid_root(x, y)

def test_calculate_unknown_operation():
    with pytest.raises(OperationError, match="Unknown operation"):
        Calculation(operation="Foo", operand1=Decimal("1"), operand2=Decimal("2"))